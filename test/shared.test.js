...
test("shared clients use existing accounts and T3 aliases without owning the proxy", async (t) => {
  ...
  assert.equal(env.ANTHROPIC_DEFAULT_HAIKU_MODEL, "gpt-6-sol");
  assert.deepEqual(config.providerInstances.claudeAgent.config.customModels, ["gpt-6-astra", "gpt-6-sol", "claude-opus-5", "claude-fable-5-1"]);
